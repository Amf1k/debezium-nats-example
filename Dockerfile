...
ARG GO_VERSION=1.23
FROM --platform=$BUILDPLATFORM golang:${GO_VERSION}-alpine AS build-app
WORKDIR /src
...
FROM --platform=$BUILDPLATFORM golang:${GO_VERSION}-alpine AS build-consumer
WORKDIR /src
...
COPY --from=build-consumer /bin/consumer /bin/
